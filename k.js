...
const robuxPackages = [
  ...
  [660,100000],[700,106000],[740,121000],[780,128000],[820,130000],
...
bot.start(ctx => {
  ...
  if (ctx.startPayload) {
    ...
    if (ref && ref !== id && db.users[ref] && !db.users[id].ref) {
      db.users[id].ref = ref;
      db.users[ref].balance += 1; // Har bir qo‘shgan foydalanuvchiga 1 Robux
      saveDB();

      ctx.telegram.sendMessage(ref, '🎁 Sizga 1 Robux referral bonus berildi!');

      // Adminlarga xabar
      ADMIN_IDS.forEach(admin => {
        ctx.telegram.sendMessage(admin, `📣 Referral\n@${db.users[ref].username} yangi foydalanuvchi qo‘shdi. 1 Robux bonus berildi.`);
      });
    }
  }
...
// ===== ADMIN =====
bot.hears('📞 Admin', ctx => {
  ctx.reply('✍️ Xabaringizni yozing, adminlarga yuboriladi.');
});
